...
import edu.davinci.lenguaje.ASTParametroFormalEntradaHilo;
import edu.davinci.lenguaje.ASTParametrosFormalesHilo;
...
/**
 * Clase encargada de representar al "hilo"
 * 
 * <p>
 ...
 */
public class TareaCompuestaProcUsuarioHilo extends TareaCompuesta {

	/** The valor parametros reales. */
	private ArrayList<Dato> valorParametrosReales;

	/**
	 * Instantiates a new tarea compuesta proc usuario hilo.
	 * 
	 * @param nodo
	 * @param contextoPadre
	 */
	public TareaCompuestaProcUsuarioHilo(SimpleNode nodo, Hashtable<String, Object> contextoPadre) {
		// inicializamos la tarea
		super(obtenerNodoProceso(nodo), crearPilaProceso(contextoPadre));
		// seteamos el nombre de la tarea (va a definir el nombre del robot)
		setNombre(((ASTProcedimientoUsuarioHilo) nodo).getNombre());
		...
		if (hayParametrosReales(nodo)) {
			...
			Node[] parametrosFormales = ((ASTParametrosFormalesHilo) getNodos().getNodo()).getChildren();

			// traemos los nombres de los parametros reales
			// List<String> nombreParametrosReales = obtenerNombreParametrosReales(nodo);

			establecerCorrespondenciaEntreParametros(parametrosFormales, getContexto(), valorParametrosReales);
	...
	/**
	 * Estable la correspondencia entre los parametros reales y los parametros formales.
	 * 
	 ...
	 */
	private void establecerCorrespondenciaEntreParametros(Node[] parametrosFormales, Hashtable<String, Object> contexto, List<Dato> valores) {
		...
		for (int i = 0; i < parametrosFormales.length; i++) {
			...
			nombre = ((ASTParametroFormalEntradaHilo) parametrosFormales[i]).getNombre();
	...
	private static SimpleNode obtenerNodoProceso(SimpleNode nodo) {
		...
		return DefinicionProcedimientos.get(((ASTProcedimientoUsuarioHilo) nodo).getNombre());
	}
