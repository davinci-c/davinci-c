...
import edu.davinci.ui.editor.configurador.EditorVisualCiudad;
import edu.davinci.ui.editor.configurador.EditorVisualInterprete;
import edu.davinci.ui.editor.configurador.Persistidor;
import edu.davinci.ui.editor.configurador.PersistidorPropiedadesCiudad;
import edu.davinci.ui.editor.configurador.PersistidorPropiedadesInterprete;
import java.io.File;

public class AccionConfigurarCiudad extends AbstractAction {

	private static final long serialVersionUID = 1L;
	private EditorSencillo editor;

	public AccionConfigurarCiudad(EditorSencillo editor) {
		super("Configurar Ciudad", getImage("images/ide/config-ciudad.png"));
		this.editor = editor;
		...
		try {
			...
			if (editor.getEjecutor() == null
					|| editor.getEjecutor().getEstado().equals(Estado.PARADO)) {
                            ...
                            String archivoConfiguracion = OpcionesInterprete.getArchivoConfiguracion();
                            
                            File archivo = new File(archivoConfiguracion);
                            
                            Persistidor persistidor = new PersistidorPropiedadesCiudad(archivo);
                            
                            EditorVisualCiudad evi = new EditorVisualCiudad(persistidor,editor);
                            evi.setVisible(true);
